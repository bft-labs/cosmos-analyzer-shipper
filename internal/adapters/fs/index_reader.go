...
func NewIndexReader(walDir string, logger ports.Logger) *IndexReader {
	return &IndexReader{
		walDir: walDir,
		logger: logger,
	}
...
func (r *IndexReader) Open(ctx context.Context, state *domain.State) error {
	var idxPath string
	var offset int64

	if state != nil && state.IdxPath != "" {
		idxPath = state.IdxPath
		offset = state.IdxOffset
	} else {
	...
	if state != nil && state.CurGz != "" {
		gzPath := filepath.Join(filepath.Dir(idxPath), state.CurGz)
...
func (r *IndexReader) Next(ctx context.Context) (domain.Frame, []byte, int, error) {
	...
	if r.gzFile == nil || r.curGz != frame.File {
		...
		if err != nil {
			return domain.Frame{}, nil, len(line), err
		}
		r.gzFile = gzf
		r.curGz = frame.File
	}
